...
# 재현·감사용 원본 데이터 (GitHub raw). "재현 가능"을 클릭 가능한 링크로 뒷받침.
REPO = os.environ.get("CLAWSAFE_REPO", "https://github.com/karmalis-prj/clawsafe")
VERDICTS_URL = os.environ.get(
    "CLAWSAFE_VERDICTS_URL",
    "https://github.com/karmalis-prj/clawsafe/blob/master/data/verdicts.jsonl",
)
...
.note{{color:var(--dim);font-size:12.5px;margin:-8px 0 22px}}
...
<div class="stat" title="3종 만장일치 malicious는 원 논문에서도 전체의 0.69%뿐 — UNSAFE가 0에 가까운 건 결함이 아니라 스캐너들이 악성엔 좀처럼 함께 확신하지 못한다는 이 논지의 증거다."><b>{unsafe}</b><span>UNSAFE (만장일치 악성)</span></div>
<div class="stat"><b>{safe}</b><span>만장일치 SAFE</span></div>
</div>
<p class="note">UNSAFE {unsafe}건 — 세 스캐너가 <b>함께</b> 악성으로 확신하는 일은 드물다(원 논문 표본에서도 0.69%). 위험은 대개 <b>불일치</b> 속에 숨는다.</p>

<div class="callout">
스캐너 3종은 서로 자주 갈린다 — 이 표본의 <b>{disp_pct}%</b>가 불일치다. 어느 하나만 믿으면 놓친다.
<a href="https://{paper}" rel="noopener">Koc 외(arxiv 2606.01494)</a>, <i>“ClawHub Security Signals: When VirusTotal, Static Analysis, and SkillSpector Disagree”</i>는
67,453개 스킬 버전에서 <b>세 스캐너 positive의 겹침이 최대 10.4%</b>, 단일 스캐너만 flag가 <b>81.9%</b>임을 보였다.
이 리더보드는 그 현상을 매일 갱신하는 <b>살아있는 축소 실증</b>이다 — ClawSafe는 불일치를 라벨로 덮지 않고 <b>증거로 보존</b>하고, 갈린 판정(DISPUTED)을 맨 위에 둔다.
</div>
...
전 판정 원본: <a href="{verdicts_url}" rel="noopener"><code>data/verdicts.jsonl</code></a> ·
<a href="{repo}" rel="noopener">소스·파이프라인</a>(표준 라이브러리만, 외부 요청 0)로 재현 가능.
</footer>
</div></body></html>"""
